...
extern char BUILT_IN_SWITCH[];
extern char CASE_ADD[];
extern char CASE_SUB[];
extern char CASE_MUL[];
extern char CASE_DIV[];
extern char CASE_MOD[];
extern char CASE_EQ[];
extern char CASE_LT[];
extern char CASE_LE[];
extern char CASE_GT[];
...
void* make_call_slot(int arity, int name) {
  ...
  mempcpy(to_replace, &val64, hole_len);
	...
	// pre-compute the operation to jump to in the assembly code
  if (name == INT_ADD_NAME) {
    val64 = CASE_ADD;
  } else if (name == INT_SUB_NAME) {
    val64 = CASE_SUB;
  } else if (name == INT_MUL_NAME) {
    val64 = CASE_MUL;
  } else if (name == INT_DIV_NAME) {
    val64 = CASE_DIV;
  } else if (name == INT_MOD_NAME) {
    val64 = CASE_MOD;
  } else if (name == INT_GT_NAME) {
    val64 = CASE_GT;
  } else if (name == INT_GE_NAME) {
    val64 = CASE_GE;
  } else if (name == INT_LT_NAME) {
    val64 = CASE_LT;
  } else if (name == INT_LE_NAME) {
    val64 = CASE_LE;
	...
	int64_t dyn_base_addr = ret;
  int64_t stc_base_addr = built_in_method_op;
  val64 = dyn_base_addr + (val64 - stc_base_addr);
	to_replace = memmem(ret, built_in_method_op_end-built_in_method_op, hole_str, hole_len);
  mempcpy(to_replace, &val64, hole_len);
